...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The particl-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: particl-cli [options] <command> [params]\n"
                "or:    particl-cli [options] -named <command> [name=value]...\n"
                "or:    particl-cli [options] help\n"
                "or:    particl-cli [options] help <command>\n"
                "\n";
...
public:
    ...
    const std::string m_help_doc{
        ...
        + strprintf("> particl-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            ...
            strPrint += " (run \"particl-cli -h\" for help or \"particl-cli listwallets\" to see which wallets are currently loaded).";
        }
    } else {
