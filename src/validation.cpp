...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    if (VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, vchAmount, cacheStore, *m_signature_cache, *txdata), &error)) {
        return std::nullopt;
    } else {
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    CAmount blockReward = GetProofOfStakeReward(params.GetConsensus(), pindex->nHeight, nFees);
    ...
    if ((!fIsGenesisBlock || fParticlMode) && !m_blockman.WriteBlockUndo(blockundo, state, *pindex)) {
        return false;
    }
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    if (!(state.nFlags & (BLOCK_STAKE_KERNEL_SPENT | BLOCK_FAILED_DUPLICATE_STAKE)) && !IsInitialBlockDownload() && ActiveTip() == pindex->pprev && m_options.signals) {
...
bool CoinStakeCache::GetCoinStake(Chainstate &chainstate, const uint256 &blockHash, CTransactionRef &tx)
{
    ...
    if (node::ReadTransactionFromBlock(pindex, 0, tx, chainstate.m_blockman)) {
