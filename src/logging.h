...
#endif
        UTIL        = (CategoryMask{1} << 25),
        BLOCKSTORAGE  = (CategoryMask{1} << 26),
        TXRECONCILIATION = (CategoryMask{1} << 27),
        SCAN        = (CategoryMask{1} << 28),
        TXPACKAGES  = (CategoryMask{1} << 29),

        SMSG        = (uint32_t)((CategoryMask{1} << 30) | (CategoryMask{1} << 11)),
        POS         = (CategoryMask{1} << 30),
        HDWALLET    = (uint32_t)(CategoryMask{1} << 31),
