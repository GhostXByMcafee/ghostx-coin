...
void SetupServerArgs(NodeContext& node)
{
    ...
    const auto defaultChainParams = CreateChainParams(*node.args, CBaseChainParams::MAIN);
    const auto testnetChainParams = CreateChainParams(*node.args, CBaseChainParams::TESTNET);
    // const auto signetChainParams = CreateChainParams(CBaseChainParams::SIGNET);
    const auto regtestChainParams = CreateChainParams(*node.args, CBaseChainParams::REGTEST);
    ...
    argsman.AddArg("-rpcport=<port>", strprintf("Listen for JSON-RPC connections on <port> (default: %u, testnet: %u, regtest: %u)", defaultBaseParams->RPCPort(), testnetBaseParams->RPCPort(), /*signetBaseParams->RPCPort(),*/ regtestBaseParams->RPCPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::RPC);
